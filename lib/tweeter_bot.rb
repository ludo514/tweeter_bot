...
def journalist(journalists)
    ...
    nombre_journalit = 5
    nouveauTableau = []
    while i < nombre_journalit
        index = rand(0..nombre_journalit)
        nouveauTableau << journalists[index]
        i+=1
    end
...
def twitte(tabl)
    i = 0
    while i < tabl.length
        login_twitter.update("Wesh #{tabl[i]} #bonjour_monde @the_hacking_pro")
        i+=1
    end
end

def like()
    login_twitter.favorite(client.search("#Hello_World", result_type:"recent").take(20))
    #user = login_twitter.user_search("#Bonjour_monde", 10)
end

def follow()
    return login_twitter.search("#Bonjour_monde", result_type: "recent").take(10).collect do |tweet|
          login_twitter.follow(tweet.user)
    end
end

def like_follow_stream()
    login_twitter_streaming.filter(track: "#Bonjour_monde") do |object|
        if object.is_a?(Twitter::Tweet) then
            print "Nouveau Tweet de : @#{object.user.screen_name} : "
            puts object.text 
            login_twitter_streaming.follow(object.user) if  login_twitter_streaming.user != login_twitter_streaming.user
            login_twitter_streaming.fav object 
        end
    end
end
...
#journalist_twitte = journalist(journalists)
#twitte(journalist_twitte)
